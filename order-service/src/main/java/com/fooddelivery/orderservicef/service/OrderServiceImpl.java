...
//    }
    ...
    private void handleStatusUpdatesAfterTransition(Order order) {
        if (order.getStatus() == OrderStatus.OUT_FOR_DELIVERY) {
            ...
            // Only update delivery status in delivery-service to DELIVERED
            // The delivery service will then handle updating the agent's status to AVAILABLE
            if (order.getDeliveryId() != null) {
                try {
                    DeliveryStatusUpdateRequestDTO deliveryStatusUpdate = DeliveryStatusUpdateRequestDTO.builder()
                            .status(DeliveryStatus.DELIVERED)
                            .estimatedDeliveryTime(LocalDateTime.now()) // Set actual delivery time
                            .build();
                    agentServiceClient.updateDeliveryStatus(order.getDeliveryId(), deliveryStatusUpdate);
                    log.info("Delivery status updated to DELIVERED in delivery-service for deliveryId={}", order.getDeliveryId());
                } catch (Exception e) {
                    log.error("Failed to update delivery status in delivery-service for orderId={}: {}", order.getOrderId(), e.getMessage());
                    // Consider retry mechanism or alerting if this is critical
            ...
            // REMOVED: Direct call to update agent status from OrderService
            // This is handled by the Delivery Service when delivery status becomes DELIVERED.
        }
